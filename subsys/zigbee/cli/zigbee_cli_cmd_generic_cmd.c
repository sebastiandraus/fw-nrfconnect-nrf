/*
 * Copyright (c) 2020 Nordic Semiconductor ASA
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#include <errno.h>
#include <shell/shell.h>

#include <zboss_api.h>
#include <zb_error_handler.h>
#include <zb_nrf_platform.h>
#include "zigbee_cli.h"
#include "zigbee_cli_utils.h"
#include "zigbee_cli_cmd_zcl.h"

/* Payload size in bytes, payload read from string is twice the size. */
#define CMD_PAYLOAD_SIZE    25
#define CMD_TABLE_SIZE      20
#define CMD_ROW_TIMEOUT_S   10

LOG_MODULE_DECLARE(cli);

/* Structure to store command's parameters to send. */
struct cmd_query {
	bool taken;
	zb_uint8_t seq_num;
	zb_uint8_t payload[CMD_PAYLOAD_SIZE];
	zb_uint8_t payload_length;
	zb_uint16_t cmd_id;
	zb_zcl_disable_default_response_t def_resp;
	const struct shell *shell;
	struct zcl_packet_info packet_info;
};

/* Array of structures to store command parameters. */
struct cmd_query cmd_data[CMD_TABLE_SIZE];

/**@brief Get the first free row in the command table and claim it,
 *        return -1 if none.
 *
 * @note Successfully acquired table row must be released
 *       by using @ref invalidate_row.
 */
static zb_int8_t acquire_row_table(void)
{
	int i;

	for (i = 0; i < CMD_TABLE_SIZE; i++) {
		if (cmd_data[i].taken == false) {
			cmd_data[i].taken = true;
			return i;
		}
	}
	return -1;
}

/**@brief Get the taken row with the selected seq_num, return -1 if none.
 * @param sernum Sequence Number to look for.
 */
static zb_int8_t get_cmd_table_row_by_sn(zb_uint8_t sernum)
{
	int i;

	for (i = 0; i < CMD_TABLE_SIZE; i++) {
		if (cmd_data[i].taken == true) {
			if (cmd_data[i].seq_num == sernum) {
				return i;
			}
		}
	}
	return -1;
}

/**@brief Free and invaidate row.
 *
 * @param row     Number of row to invalidate.
 */
static void invalidate_row(zb_uint8_t row)
{
	if (row < CMD_TABLE_SIZE) {
		ZB_MEMSET(&(cmd_data[row]), 0x00, sizeof(struct cmd_query));
	}
}

/**@brief Invaidate row after the timeout - ZBOSS callback.
 *
 * @param row     Number of row to invalidate.
 */
static void invalidate_row_cb(zb_uint8_t row)
{
	zb_cli_print_error(cmd_data[row].shell, "Request timed out", ZB_FALSE);
	invalidate_row(row);
}

/**@brief Check if the frame we received is the response to our request
 *        in the table.
 *
 * @param zcl_hdr            Pointer to the parsed header of the frame.
 * @param cmd_query_entry    Pointer to the row in the table to check against.
 *
 * @return Whether it is response or not.
 */
static zb_bool_t is_response(zb_zcl_parsed_hdr_t *zcl_hdr,
			     struct cmd_query *cmd_query_entry)
{
	zb_uint16_t remote_node_short = 0;
	struct zcl_packet_info *packet_info = &cmd_query_entry->packet_info;

	if (packet_info->dst_addr_mode == ZB_APS_ADDR_MODE_64_ENDP_PRESENT) {
		remote_node_short = zb_address_short_by_ieee(
					packet_info->dst_addr.addr_long);
	} else {
		remote_node_short = packet_info->dst_addr.addr_short;
	}

	if (zcl_hdr->cluster_id != packet_info->cluster_id) {
		return ZB_FALSE;
	}

	if (zcl_hdr->profile_id != packet_info->prof_id) {
		return ZB_FALSE;
	}

	if (zcl_hdr->addr_data.common_data.src_endpoint !=
	    packet_info->dst_ep) {
		return ZB_FALSE;
	}

	if (zcl_hdr->addr_data.common_data.source.addr_type ==
	    ZB_ZCL_ADDR_TYPE_SHORT) {
		if (zcl_hdr->addr_data.common_data.source.u.short_addr !=
		    remote_node_short) {
			return ZB_FALSE;
		}
	} else {
		return ZB_FALSE;
	}

	if (zcl_hdr->cmd_id != ZB_ZCL_CMD_DEFAULT_RESP) {
		return ZB_FALSE;
	}

	return ZB_TRUE;
}

/**@brief Send the ZCL command frame.
 *
 * @param param  Row of the command parameter table.
 */
static void zcl_cmd_send(zb_uint8_t param)
{
	zb_ret_t zb_err_code;
	struct cmd_query *cmd_query_entry = &cmd_data[param];

	/* Send the actual frame. */
	zb_err_code = zb_zcl_finish_and_send_packet_new(
				cmd_query_entry->packet_info.buffer,
				cmd_query_entry->packet_info.ptr,
				&(cmd_query_entry->packet_info.dst_addr),
				cmd_query_entry->packet_info.dst_addr_mode,
				cmd_query_entry->packet_info.dst_ep,
				cmd_query_entry->packet_info.ep,
				cmd_query_entry->packet_info.prof_id,
				cmd_query_entry->packet_info.cluster_id,
				cmd_query_entry->packet_info.cb,
				0,
				cmd_query_entry->packet_info.disable_aps_ack,
				0);

	if (zb_err_code != RET_OK) {
		zb_cli_print_error(cmd_query_entry->shell,
				   "Can not send ZCL frame", ZB_FALSE);
		/* Invalidate row ctx so that we can reuse it. */
		invalidate_row(param);
		zb_buf_free(cmd_query_entry->packet_info.buffer);
		return;
	}

	/* If default response is requested, schedule an alarm. */
	if (cmd_query_entry->def_resp == ZB_ZCL_ENABLE_DEFAULT_RESPONSE) {
		zb_err_code = ZB_SCHEDULE_APP_ALARM(invalidate_row_cb, param,
						    CMD_ROW_TIMEOUT_S *
						     ZB_TIME_ONE_SECOND);

		/* If no free buffer, print error and invalidate buffer
		 * to disable response from def_resp_cb.
		 */
		if (zb_err_code != RET_OK) {
			zb_cli_print_error(cmd_query_entry->shell,
					   "No free buffer", ZB_FALSE);
			invalidate_row(param);
		}
	} else {
		zb_cli_print_done(cmd_query_entry->shell, ZB_FALSE);
		invalidate_row(param);
	}
}

/**@brief Send generic command to the remote node.
 *
 * @code
 * zcl cmd [-d] <h:dst_addr> <d:ep> <h:cluster> [-p h:profile] <h:cmd_ID>
 *         [-l h:payload]
 * @endcode
 *
 * Send generic command with ID `cmd_ID` with payload `payload` to the cluster
 * `cluster`. The cluster belongs to the profile `profile`, which resides
 * on the endpoint `ep` of the remote node `dst_addr`. Optional default
 * response can be set with `-d`.
 *
 * @note By default profile is set to Home Automation Profile
 * @note By default payload is empty
 * @note To send via binding table, set `dst_addr` and `ep` to 0.
 *
 */
int cmd_zb_generic_cmd(const struct shell *shell, size_t argc, char **argv)
{
	ARG_UNUSED(argc);

	struct cmd_query *cmd_query_entry = NULL;
	char **arg = &argv[1];
	zb_ret_t zb_err_code;
	zb_int8_t table_row;
	zb_bufid_t bufid;
	int ret_val;
	size_t len;

	table_row = acquire_row_table();
	if (table_row < 0) {
		zb_cli_print_error(shell, "No free buffer. Wait a bit",
				   ZB_FALSE);
		return -ENOEXEC;
	}
	cmd_query_entry = &cmd_data[table_row];

	/*  Set default command values. */
	cmd_query_entry->def_resp = ZB_ZCL_DISABLE_DEFAULT_RESPONSE;
	cmd_query_entry->payload_length = 0;
	cmd_query_entry->packet_info.prof_id = ZB_AF_HA_PROFILE_ID;

	/* Check if default response should be requested. */
	if (strcmp(*arg, "-d") == 0) {
		cmd_query_entry->def_resp = ZB_ZCL_ENABLE_DEFAULT_RESPONSE;
		arg++;
	}

	/* Parse and check remote node address. */
	cmd_query_entry->packet_info.dst_addr_mode =
		parse_address(*arg, &(cmd_query_entry->packet_info.dst_addr),
			      ADDR_ANY);

	if (cmd_query_entry->packet_info.dst_addr_mode == ADDR_INVALID) {
		/* Handle the Binding table case (address == '0'). */
		if (!strcmp(*arg, "0")) {
			cmd_query_entry->packet_info.dst_addr_mode =
				ZB_APS_ADDR_MODE_DST_ADDR_ENDP_NOT_PRESENT;
			cmd_query_entry->packet_info.dst_addr.addr_short = 0;
		} else {
			zb_cli_print_error(shell, "Wrong address format",
					   ZB_FALSE);
			goto error;
		}
	}
	arg++;

	/* Read endpoint and cluster id. */
	ret_val = zb_cli_sscan_uint8(*(arg++),
				     &(cmd_query_entry->packet_info.dst_ep));
	if (ret_val == 0) {
		zb_cli_print_error(shell, "Remote ep value", ZB_FALSE);
		goto error;
	}

	ret_val = parse_hex_u16(*(arg++),
				&(cmd_query_entry->packet_info.cluster_id));
	if (ret_val == 0) {
		zb_cli_print_error(shell, "Cluster id value", ZB_FALSE);
		goto error;
	}

	/* Check if different from HA profile should be used. */
	if (strcmp(*arg, "-p") == 0) {
		ret_val =
			parse_hex_u16(*(++arg),
				      &(cmd_query_entry->packet_info.prof_id));
		if (ret_val == 0) {
			zb_cli_print_error(shell, "Profile id value", ZB_FALSE);
			goto error;
		}
		arg++;
	}

	/* Read command id. */
	ret_val = parse_hex_u16(*(arg++), &(cmd_query_entry->cmd_id));
	if (ret_val == 0) {
		zb_cli_print_error(shell, "Cmd id value", ZB_FALSE);
		goto error;
	}

	/* Check if payload should be sent. */
	if (strcmp(*(arg++), "-l") == 0) {
		len = strlen(*arg);
		if (len > (2 * CMD_PAYLOAD_SIZE)) {
			shell_warn(shell, "Payload length is too big, trimming it to first %d bytes",
				   CMD_PAYLOAD_SIZE);
			len = (2 * CMD_PAYLOAD_SIZE);
		}
		if ((len != 0) && (len % 2) != 0) {
			zb_cli_print_error(shell, "Payload length is not even",
					   ZB_FALSE);
			goto error;
		}
		ret_val = parse_hex_str(*arg, len, cmd_query_entry->payload,
					CMD_PAYLOAD_SIZE, false);
		if (ret_val == 0) {
			zb_cli_print_error(shell, "Payload value", ZB_FALSE);
			goto error;
		}
		cmd_query_entry->payload_length = len / 2;
	}

	/* Set shell to print logs to. */
	cmd_query_entry->shell = shell;

	/* Make sure ZBOSS buffer API is called safely. */
	zb_osif_disable_all_inter();
	bufid = zb_buf_get_out();
	zb_osif_enable_all_inter();

	if (!bufid) {
		zb_cli_print_error(cmd_query_entry->shell,
				   "Can not get buffer.", ZB_FALSE);
		/* Mark data structure as free. */
		invalidate_row(table_row);
		return -ENOEXEC;
	}

	/* Get the ZCL packet sequence number. */
	cmd_query_entry->seq_num = ZCL_CTX().seq_number;

	/* Start filling buffer with packet data. */
	zb_uint8_t *ptr = ZB_ZCL_START_PACKET_REQ(bufid)

	ZB_ZCL_CONSTRUCT_SPECIFIC_COMMAND_REQ_FRAME_CONTROL(
		ptr, (cmd_query_entry->def_resp))
	ZB_ZCL_CONSTRUCT_COMMAND_HEADER_REQ(ptr, ZB_ZCL_GET_SEQ_NUM(),
					    cmd_query_entry->cmd_id);

	/* Scan argument with payload and put in buffer with command to send. */
	for (zb_uint8_t i = 0; i < cmd_query_entry->payload_length; i++) {
		ZB_ZCL_PACKET_PUT_DATA8(ptr, (cmd_query_entry->payload[i]));
	}

	/* Schedule frame to send. */
	cmd_query_entry->packet_info.buffer = bufid;
	cmd_query_entry->packet_info.ptr = ptr;
	/* DstAddr, Dst Addr Mode and Dst endpoint are already set. */
	cmd_query_entry->packet_info.ep = zb_cli_get_endpoint();
	/* Profile ID and Cluster ID are already set. */
	cmd_query_entry->packet_info.cb = NULL;
	cmd_query_entry->packet_info.disable_aps_ack = ZB_FALSE;

	zb_err_code = ZB_SCHEDULE_APP_CALLBACK(zcl_cmd_send, table_row);

	if (zb_err_code != RET_OK) {
		zb_cli_print_error(cmd_query_entry->shell,
				   "Can not schedule ZCL frame", ZB_FALSE);

		/* Make sure ZBOSS buffer API is called safely. */
		zb_osif_disable_all_inter();
		zb_buf_free(cmd_query_entry->packet_info.buffer);
		zb_osif_enable_all_inter();

		invalidate_row(table_row);
		return -ENOEXEC;
	}

	return 0;

error:
		/* Mark data structure as free. */
		invalidate_row(table_row);
		return -EINVAL;
}

/**@brief Send raw ZCL frame.
 *
 * @code
 * zcl raw <h:eui64> <d:ep> <h:cluster> <h:profile> <h:payload>
 * @endcode
 *
 * Send raw ZCL frame with payload `payload` to the cluster `cluster`.
 * The cluster belongs to the profile `profile`, which resides
 * on the endpoint `ep` of the remote node `dst_addr`. The payload represents
 * the ZCL Header plus ZCL Payload.
 *
 * @note To send via binding table, set `eui64` and `ep` to 0.
 *
 */
int cmd_zb_zcl_raw(const struct shell *shell, size_t argc, char **argv)
{
	ARG_UNUSED(argc);

	zb_ret_t zb_err_code;
	zb_int8_t table_row;
	zb_bufid_t bufid;
	int ret_val;
	size_t len;
	char **arg = &argv[1];
	struct cmd_query *cmd_query_entry = NULL;

	/* Debug mode quick return. */
	if (!zb_cli_debug_get()) {
		zb_cli_print_error(shell, "This command is available only in debug mode. Run 'debug on' to enable it",
				   ZB_FALSE);
		return -ENOEXEC;
	}

	table_row = acquire_row_table();
	if (table_row < 0) {
		zb_cli_print_error(shell, "No free buffer. Wait a bit",
				   ZB_FALSE);
		return -ENOEXEC;
	}
	cmd_query_entry = &cmd_data[table_row];

	/*  Reset the counter. */
	cmd_query_entry->payload_length = 0;

	/* Parse and check remote node address. */
	cmd_query_entry->packet_info.dst_addr_mode =
		parse_address(*arg, &(cmd_query_entry->packet_info.dst_addr),
			      ADDR_ANY);

	if (cmd_query_entry->packet_info.dst_addr_mode == ADDR_INVALID) {
		/* Handle the Binding table case (address == '0'). */
		if (!strcmp(*arg, "0")) {
			cmd_query_entry->packet_info.dst_addr_mode =
				ZB_APS_ADDR_MODE_DST_ADDR_ENDP_NOT_PRESENT;
			cmd_query_entry->packet_info.dst_addr.addr_short = 0;
		} else {
			zb_cli_print_error(shell, "Wrong EUI64 address format",
					   ZB_FALSE);
			goto error;
		}
	}
	arg++;

	/* Read endpoint and cluster id. */
	ret_val = zb_cli_sscan_uint8(*(arg++),
				     &(cmd_query_entry->packet_info.dst_ep));
	if (ret_val == 0) {
		zb_cli_print_error(shell, "Remote ep value", ZB_FALSE);
		goto error;
	}

	ret_val = parse_hex_u16(*(arg++),
				&(cmd_query_entry->packet_info.cluster_id));
	if (ret_val == 0) {
		zb_cli_print_error(shell, "Cluster id value", ZB_FALSE);
		goto error;
	}

	ret_val = parse_hex_u16(*(arg++),
				&(cmd_query_entry->packet_info.prof_id));
	if (ret_val == 0) {
		zb_cli_print_error(shell, "Profile id value", ZB_FALSE);
		goto error;
	}

	/* Reuse the payload field from the context. */
	len = strlen(*arg);
	if (len > (2 * CMD_PAYLOAD_SIZE)) {
		shell_warn(shell, "Raw data length is too big, trimming it to first %d bytes",
			   CMD_PAYLOAD_SIZE);
		len = (2 * CMD_PAYLOAD_SIZE);
	}

	if ((len != 0) && (len % 2) != 0) {
		zb_cli_print_error(shell, "Payload length is not even",
				   ZB_FALSE);
		goto error;
	}

	ret_val = parse_hex_str(*arg, len, cmd_query_entry->payload,
				CMD_PAYLOAD_SIZE, false);
	if (ret_val == 0) {
		zb_cli_print_error(shell, "Payload value", ZB_FALSE);
		goto error;
	}
	cmd_query_entry->payload_length = len / 2;

	/* Disable default response for zcl raw command. */
	cmd_query_entry->def_resp = ZB_ZCL_DISABLE_DEFAULT_RESPONSE;

	/* Set shell to print logs to. */
	cmd_query_entry->shell = shell;

	/* Make sure ZBOSS buffer API is called safely. */
	zb_osif_disable_all_inter();
	bufid = zb_buf_get_out();
	zb_osif_enable_all_inter();

	if (!bufid) {
		zb_cli_print_error(cmd_query_entry->shell,
				   "Can not get buffer.", ZB_FALSE);
		/* Mark data structure as free. */
		invalidate_row(table_row);
		return -ENOEXEC;

	}
	/* Get the ZCL packet sequence number. */
	cmd_query_entry->seq_num = ZCL_CTX().seq_number;

	/* Start filling buffer with packet data. */
	zb_uint8_t *ptr = ZB_ZCL_START_PACKET(bufid);

	/* Scan argument with payload and put in buffer with command to send. */
	for (zb_uint8_t i = 0; i < cmd_query_entry->payload_length; i++) {
		ZB_ZCL_PACKET_PUT_DATA8(ptr, (cmd_query_entry->payload[i]));
	}

	/* Schedle frame to send. */
	cmd_query_entry->packet_info.buffer = bufid;
	cmd_query_entry->packet_info.ptr = ptr;
	/* DstAddr, Dst Addr Mode and Dst endpoint are already set. */
	cmd_query_entry->packet_info.ep = zb_cli_get_endpoint();
	/* Profile ID and Cluster ID are already set. */
	cmd_query_entry->packet_info.cb = NULL;
	cmd_query_entry->packet_info.disable_aps_ack = ZB_FALSE;

	zb_err_code = ZB_SCHEDULE_APP_CALLBACK(zcl_cmd_send, table_row);

	if (zb_err_code != RET_OK) {
		zb_cli_print_error(cmd_query_entry->shell,
				   "Can not schedule ZCL frame", ZB_FALSE);

		/* Make sure ZBOSS buffer API is called safely. */
		zb_osif_disable_all_inter();
		zb_buf_free(cmd_query_entry->packet_info.buffer);
		zb_osif_enable_all_inter();

		invalidate_row(table_row);
		return -ENOEXEC;
	}

	return 0;

error:
		invalidate_row(table_row);
		return -EINVAL;
}

/**@brief The Handler to 'intercept' every frame coming to the endpoint.
 *
 * @param bufid    Reference to a ZBOSS buffer.
 */
zb_uint8_t cli_agent_ep_handler_generic_cmd(zb_bufid_t bufid)
{
	zb_int8_t row;
	zb_ret_t zb_err_code;
	zb_zcl_parsed_hdr_t *cmd_info = ZB_BUF_GET_PARAM(bufid,
							 zb_zcl_parsed_hdr_t);

	/* Get the row in the requests table according to the seq number. */
	row = get_cmd_table_row_by_sn(cmd_info->seq_number);
	if (row == -1) {
		return ZB_FALSE;
	}

	struct cmd_query *cmd_query_entry = &(cmd_data[row]);

	if (!is_response(cmd_info, cmd_query_entry)) {
		return ZB_FALSE;
	}

	if (cmd_info->cmd_id == ZB_ZCL_CMD_DEFAULT_RESP) {
		zb_zcl_default_resp_payload_t *def_resp;

		def_resp = ZB_ZCL_READ_DEFAULT_RESP(bufid);

		/* Print info received from default response. */
		shell_fprintf(cmd_query_entry->shell, (def_resp->status ==
			       ZB_ZCL_STATUS_SUCCESS) ? SHELL_INFO :
			       SHELL_ERROR, "Default Response received: ");
		shell_fprintf(cmd_query_entry->shell,
			      (def_resp->status == ZB_ZCL_STATUS_SUCCESS) ?
			       SHELL_INFO : SHELL_ERROR,
			      "Command: %d, Status: %d\n",
			      def_resp->command_id, def_resp->status);

		if (def_resp->status != ZB_ZCL_STATUS_SUCCESS) {
			zb_cli_print_error(cmd_query_entry->shell,
					   "Command not successful", ZB_FALSE);
		} else {
			zb_cli_print_done(cmd_query_entry->shell, ZB_FALSE);
		}
	} else {
		/* In case of unknown response. */
		zb_cli_print_error(cmd_query_entry->shell, "Unknown response",
				   ZB_FALSE);
	}
	/* Cancel the ongoing alarm which was to erase the row... */
	if (cmd_data[row].def_resp == ZB_ZCL_ENABLE_DEFAULT_RESPONSE) {
		zb_err_code = (ZB_SCHEDULE_APP_ALARM_CANCEL(invalidate_row_cb,
							    row));
		ZB_ERROR_CHECK(zb_err_code);
	}
	/* ...and erase it manually. */
	invalidate_row(row);

	zb_buf_free(bufid);
	return ZB_TRUE;
}
