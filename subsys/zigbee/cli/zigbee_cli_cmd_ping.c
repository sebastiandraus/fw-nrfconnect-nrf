...
zb_void_t zb_ping_release_request(struct ping_request *ping_reply_ctx)
{
 ...
 */
zb_void_t ping_release_reply(struct ping_reply *ping_reply_ctx)
{
	if (ping_reply_ctx != NULL) {
		ping_reply_ctx->taken = false;
...
zb_void_t ping_request_send(struct ping_request *ping_req_ctx)
{
	zb_ret_t zb_err_code;
	zb_bufid_t bufid;
 ...
 */
static zb_void_t ping_reply_send(struct ping_reply *ping_reply_ctx)
{
	zb_bufid_t bufid;
	zb_uint8_t *cmd_buf_ptr;
	zb_uint8_t cli_ep = zb_cli_get_endpoint();
